import React from 'react'
import { DocListItem } from '@/components/DocListItem'
import { AccessibilityTrigger } from '@/components/AccessibilityTrigger'

export default function PlansAndReportPage() {
  return (
    <div className="w-layout-blockcontainer o-container department header-trigger w-container">
      <AccessibilityTrigger />
      
      <h1 className="plans-header">Плани та звіти діяльності ДАРТ</h1>
      <div className="o-tabpane-inf">
        <div className="plans-links">
          <DocListItem href="/webflow/documents/dart_report_2025_web.pdf" label="Публічний звіт Голови Державного агентства розвитку туризму України Наталі Табаки за 2025 рік" />
        </div>
        <div className="plans-links">
          <DocListItem href="/webflow/documents/Публічний-звіт-ДАРТ_2024.pdf" label="Публічний звіт Державного агентства розвитку туризму України за 2024 рік" />
        </div>
        <div className="plans-links">
          <DocListItem href="/webflow/documents/Орієнтовний-план_-проведення-консультацій-з-громадськістю-на-2025-рік.pdf" label="Орієнтовний план проведення консультацій з громадськістю на 2025 рік" />
        </div>
        <div className="plans-links">
          <DocListItem href="/webflow/documents/Звіт_ПІ_2024.pdf" label="Звіт про результати розглядів запитів на інформацію у 2024 році." />
        </div>
        <div className="plans-links">
          <DocListItem href="/webflow/documents/consultations-with-people2024.pdf" label="Орієнтовний план проведення консультацій з громадськістю на 2024 рік (зі змінами)" />
        </div>
        <div className="plans-links">
          <DocListItem href="https://drive.google.com/file/d/1jY1JlrAb9wHvSHi27tAVr2sn2Acz8SUn/view" label="План діяльності з внутрішнього аудиту на 2023-2025 роки" />
        </div>
        <div className="plans-links">
          <DocListItem href="/webflow/documents/план2024_План_регулятор.pdf" label="План діяльності Державного агентства розвитку туризму України з підготовки проектів регуляторних актів на 2024 рік" />
        </div>
        <div className="plans-links">
          <DocListItem href="https://drive.google.com/file/d/1-Ma8w8U3R0L0jx8lFP6sVj4-V1EbouTw/view?usp=sharing" label="Орієнтовний план проведення консультацій з громадськістю на 2024 рік" />
        </div>
        <div className="plans-links">
          <DocListItem href="https://drive.google.com/file/d/1EslhLM24JiwttAI-rcUQSbm3y525FqSR/view" label="Звіт про результати розглядів запитів на інформацію у 2023 році." />
          <DocListItem href="https://drive.google.com/file/d/1ixkNelbBVc7vyV3buYUyS29sCkvam-Tk/view" label="Публічний звіт Голови Державного агентства розвитку туризму України Мар’яни Олеськів за 2023 рік" />
        </div>
        <div className="plans-links">
          <DocListItem href="https://drive.google.com/file/d/1RFqdJmPRVGjdF6ezjDhD5q8488P_sBIi/view?usp=sharing" label="План діяльності Державного агентства розвитку туризму України з підготовки проєктів регуляторних актів на 2023 рік" />
        </div>
        <div className="plans-links">
          <DocListItem href="https://drive.google.com/file/d/1ohkdZbK7GfoYF6x7R2DCAGkUUdZOWlwp/view" label="Орієнтовний план проведення консультацій з громадськістю на 2022 рік" />
        </div>
        <div className="plans-links">
          <DocListItem href="https://drive.google.com/file/d/10hchIm4hcwTGqwA3cEYBKVmryRocP1Dn/view?usp=sharing" label='Наказ № 310 від 31.12.2021 "Про затвердження Орієнтовного плану проведення консультацій з громадськістю на 2022 рік"' />
        </div>
        <div className="plans-links">
          <DocListItem href="https://drive.google.com/file/d/1AtcECZxYteZfkEsjXF0GbNHM9n1VCsxN/view" label="Публічний звіт Голови Державного агентства розвитку туризму України Мар'яни Олеськів за 2022 рік" />
        </div>
        <div className="plans-links">
          <DocListItem href="https://drive.google.com/file/d/1ajBWS7OXTRN5fn39Ech55ZH2ruv5K9do/view?usp=sharing" label="Стратегічний план діяльності з внутрішнього аудиту на 2022 – 2024 роки" />
        </div>
        <div className="plans-links">
          <DocListItem href="/webflow/documents/ЗВІТ-2021.pdf" label="Звіт Державного агенства розвитку туризму України за 2021 рік" />
        </div>
        <div className="plans-links">
          <DocListItem href="https://drive.google.com/file/d/1DywX2UVhHl4I_CR9__1ec4DqEV_sQswR/view?usp=sharing" label="Операційний план з внутрішнього аудиту на 2022" />
        </div>
        <div className="plans-links">
          <DocListItem href="https://drive.google.com/file/d/1P5igH0wNg-9x0coit2KLirXfir0FPYoi/view?usp=sharing" label="Кошторис на 2021 рік (28.10.2021)" />
        </div>
        <div className="plans-links">
          <DocListItem href="https://drive.google.com/file/d/1TN1MfeIu4NJemHvXqkdR3vhrsZfRPbi4/view?usp=sharing" label='Наказ №619 від 16.11.2021 "Про затвердження бюджетної програми на 2021 за КПКВК 3106010' />
        </div>
        <div className="plans-links">
          <DocListItem href="https://drive.google.com/file/d/1ItST6nVoHYGVbvnTLe-tRgxZc6iIQk60/view?usp=sharing" label="Штатний розпис" />
        </div>
        <div className="plans-links">
          <DocListItem href="https://drive.google.com/file/d/1NA2dR7yi6Wx14chQlxL_gOTZyuE1cjrZ/view?usp=sharing" label="Кошторис на 2021 рік (28 січня 2021)" />
        </div>
        <div className="plans-links">
          <DocListItem href="https://drive.google.com/file/d/1TYD7MZZ0yRN1GRInrd4XU2vO7WOuBnyE/view?usp=sharing" label="План діяльності Державного агентства розвитку туризму України на 2021 рік" />
        </div>
        <div className="plans-links">
          <DocListItem href="/webflow/documents/звіт-ДАРТ-20201_compressed.pdf" label="Звіт Державного агенства розвитку туризму України за 2020 рік" />
        </div>
        <div className="plans-links">
          <DocListItem href="/webflow/documents/52_план-ДАРТ.pdf" label="План діяльності Державного агенства розвитку туризму України на серпень — грудень 2020 року" />
        </div>
      </div>
    </div>
  )
}
